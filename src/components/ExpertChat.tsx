...
import { Send, MessageCircle, Sparkles, ShieldCheck, Bot, CheckCircle2, UserRound } from 'lucide-react';
...
export const ExpertChat: React.FC<ExpertChatProps> = ({ lang, onSendChatMessage }) => {
  ...
  const handleSendMessage = async (suggestedQuestion?: string) => {
    ...
    setChatResponse('');
    try {
      setChatResponse(await onSendChatMessage(userMessage));
    } catch {
    ...
    <section className="max-w-6xl mx-auto animate-fade-in space-y-8" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1.65fr)_minmax(280px,0.75fr)] gap-6 items-start">
        <div className="rounded-[2rem] border border-white/10 bg-[#091833]/80 p-5 sm:p-8 shadow-2xl">
          ...
          <button type="button" onClick={() => handleSendMessage()} disabled={isChatting || !chatInput.trim()}
            className="mt-3 min-h-[50px] w-full rounded-2xl bg-gradient-to-r from-[#F5A623] to-[#FF8A00] px-5 py-3 font-black text-[#030914] shadow-lg shadow-[#F5A623]/15 transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-45 flex items-center justify-center gap-2">
          ...
          <div aria-live="polite" aria-busy={isChatting}>
            {(isChatting || chatResponse) ? (
              <div className={`mt-7 rounded-2xl border border-[#F5A623]/25 bg-[#030914]/75 p-5 sm:p-6 ${alignClass}`}>
                ...
                {isChatting ? <p className="animate-pulse text-sm font-bold text-[#94A3B8]">{copy.thinking}</p> : <div>{formatText(chatResponse)}</div>}
              </div>
            ) : null}
          </div>
        </div>
